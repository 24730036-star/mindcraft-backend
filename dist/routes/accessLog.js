...
router.post("/", async (req, res) => {
    try {
        ...
        const nextId = await (0, id_1.getNextId)("AccessLog");
        await (0, sheets_1.appendRow)("AccessLog", [
            String(nextId), // id (A열)
...
router.get("/interest-summary", async (req, res) => {
    try {
        ...
        const [accessRowsRaw, storiesRowsRaw, usersRowsRaw] = await Promise.all([
            (0, sheets_1.readSheet)("AccessLog"),
            (0, sheets_1.readSheet)("Stories"),
            (0, sheets_1.readSheet)("Users"),
        ]);
        const accessRows = accessRowsRaw !== null && accessRowsRaw !== void 0 ? accessRowsRaw : [];
        const storiesRows = storiesRowsRaw !== null && storiesRowsRaw !== void 0 ? storiesRowsRaw : [];
        const usersRows = usersRowsRaw !== null && usersRowsRaw !== void 0 ? usersRowsRaw : [];
        // 해당 스토리에 대한 INTEREST 기록만 필터링
        const interestRows = accessRows.filter((row) => Number(row[1]) === storyId && row[3] === "INTEREST");
        ...
        // 요청자가 글 작성자인지 확인
        if (requesterId) {
            const storyRow = storiesRows.find((row) => Number(row[0]) === storyId);
